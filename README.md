# This forked repository contains new functionality for the TIP3 interface with the allowance feature.

## Motivation
The following fork version extends TIP-3.2 with ‘allowance’ functionality which is commonly used in traditional token standards, such as ERC-20.The TIP-3.2 standard describes how to create a token wallet, how token wallets are owned, how to transfer, mint or burn tokens, how tokens recipients can handle the incoming transfer, etc. Otherwise it doesn’t describe an allowance interface by which token owners can allow a spender contract to spend a certain amount of tokens on their behalf to add support of pull payments (e.g. subscriptions, regular payouts, instalments, costs averaging etc).


# Technical desciption of the implementation:
